...
**<font color=red>ZSet 可以实现 延时消息队列</font>， 将时间戳 所谓score存储， 查询时 指定时间范围即可**

```bash

# 返回分数在[min,max]之间的元素（默认闭区间）
...
- 分数相同的元素按**字典序**排列  即 **<font color=green>member 的 字符的编码值  逐个比较</font>**
- 中文需转为统一编码（如UTF-8）才能正确排序
...
### 2.2 HyperLogLog  -- 高级数据结构(本质是一个字符串)
...
#### 3.2.3 Redis 与 LUA
**Lua 脚本是 Redis 提供的一种强大功能，允许用户在服务器端执行复杂的<font color=red>原子性操作</font>**。
...
- Key分散(子key)
  ...
  - ```bash
    ...
    product:12345:info
    # 拆分为
    product:12345:info:part1
    product:12345:info:part2
    ```
- Redisson 提供了热key解决方案

  - 自定义热键监控器

  ```java
  @Bean
  public HotKeyMonitor hotKeyMonitor(RedissonClient redisson) {
      return new RedissonHotKeyMonitor(redisson)
          .withThreshold(500)  // 每秒500次访问视为热键
          .withSampleRate(0.2) // 20%采样率
          .withReportInterval(1, TimeUnit.MINUTES); // 每分钟报告
  }
  ```
  ...
  ```java
  RLocalCachedMap<String, Object> map = redisson.getLocalCachedMap("hotKeys",
