...
| 索引类型      | 数据结构       | 是否有序 | 支持查询类型               | 存储方式     | 适用场景                   |
| :------------ | :------------- | :------- | :------------------------- | :----------- | :------------------------- |
| **B-Tree**    | 平衡多路搜索树 | 是       | 全值/范围/前缀/排序        | 磁盘连续存储 | 常规OLTP（默认索引类型）   |
| **B+Tree**    | B-Tree变种     | 是       | 全值/范围/前缀/排序        | 磁盘连续存储 | InnoDB聚簇索引（主键索引） |
| **Hash**      | 哈希表         | 否       | 仅精确匹配(=, IN)          | 内存为主     | 内存表/等值查询高频场景    |
| **Full-Text** | 倒排索引       | 部分     | 文本搜索(MATCH AGAINST)    | 特殊文件结构 | 文本内容搜索               |
| **R-Tree**    | 空间索引       | 区域     | 空间关系(Contains, Within) | 专用存储结构 | GIS地理数据                |
| **前缀索引**  | B+Tree子集     | 是       | 前缀匹配(LIKE 'abc%')      | 部分列值存储 | 长字符串列                 |
...
**<font color=#ff008a>Repeatable Read </font> --- <font color=green>默认的数据隔离级别</font>**
...
1. **<font color=green>消除回表操作</font> : <font color=orange> select * from user inner join (select id from user where age > 18 limit 100000, 10) u ON user.id = u.id;</font>**
2. **<font color=green>唯一递增列 记录上一页的最大id</font> select * from user where id > lastID limit 10;**
3. 业务层控制 不进行深度分页 或者 es来解决