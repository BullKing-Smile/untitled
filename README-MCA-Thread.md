...
Java Memory Model Java内存模型是一个**<font color=green>语言级别的内存模型抽象</font>**，屏蔽了底层硬件实现内存一致性需求的差异， 提供了对上层统一的接口来保证内存一致性的编程能力。

Java 是一个跨平台语言， java内存模型就是一个中间层模式，他**适配了不同的底层硬件系统，设计中间层模型来做屏蔽**。
...
### 2.2 保证并发编程的原子性
#### 2.2.1 synchronized 同步代码块的方式 保证原子性

#### 2.2.2 CAS  性能比synchronized效率高
...
#### 2.2.3 Lock 并发较大的情况下 性能比synchronized更优
...
#### 2.3.1 volatile是一个关键字， 用来修饰成员变量。
如果属性被volatile修饰，**<font color=red>相当于告诉CPU，对当前属性的操作，不允许使用CPU的缓存，必须去和主内存操作</font>**。

volatile 内存语义
- volatile属性被写：当写一个volatile变量，JMM会将当前线程对应的CPU缓存及时刷新到主内存中。
- volatile属性被读：当读一个volatile变量，JMM会将对应的CPU缓存中的内存设置为无效，必须去主内存中读取共享变量。

其实加了volatile就是**<font color=green>告知CPU，对当前属性的读写操作，不允许使用CPU缓存</font>**，加了volatile修饰的属性，会再转为汇编之后，追加一个lock前缀，CPU执行这个指令时，如果带有lock前缀会做两个事情：
- 将当前处理器缓存行的数据写回到主内存
- 这个写回的数据，在其他CPU内核的缓存中，直接无效

#### 2.3.2 synchronized 同时可以解决可见性问题（也解决原子性问题）
...
Java中提供的**CAS操作，就是乐观锁的一种实现**。 Atomic原子性类中 就是基于CAS乐观锁的实现。        
...
- 同步队列（双向链表）：获取ReentrantLock锁资源，当前锁资源被其他线程池有了，当前线程就需要排队等待，在同步队列中去排队。
...
### 4.2 newFixedThreadPool 线程个数是固定的线程池， 线程是懒加载
...
### 4.5 newScheduleThreadPool() --- 定时任务的线程池， 基于DelayQueue实现的延迟执行。
...
### 4.6 newWorkStealingPool 与之前的线程池 有很大的区别。
...
#### 4.8.3 ThreadPoolExecutor.execute(Runnable) 方法介绍
线程池提交任务的核心方法
...
## 5 JUC - java.util.concurrent 并发编程


### 5.1 ConcurrentHashMap 线程安全的key-value集合
...
如果链表大于8 但数组长度 小于 64, 进行resize() --- 扩容操作


**<font color=red>为什么 链表长度的阈值为8？</font>**</br>
泊松分布（Poisson Distribution）—— 哈希冲突的概率</br>
**<font color=green>哈希冲突的概率（链表长度 ≥8）极低（约 0.00000006，即千万分之六）</font>**
...
### 5.2 ConcurrentHashMap - CHM 扩容与协助扩容
...
### 5.3 ConcurrentHashMap - CHM查询数据流程
ConcurrentHashMap本来就是为了JVM缓存使用的。 对查询速度要求极高。所以查询操作永远不会阻塞。


### 5.4 ConcurrentHashMap - CHM 数据插入流程
...
### 5.6 CHM TreeBin的锁操作
- **<font color=red>如果读线程正在读取红黑树的数据， 写线程要阻塞</font>**
- **<font color=green>如果写线程正在操作红黑树（做平衡），读线程不会阻塞，会读取双向链表的数据</font>**
- **<font color=orange>读读不会阻塞！</font>**
- **<font color=blue>写写，不存在并发操作，因为操作桶这一步已经synchronized 了</font>**
- 



### 5.7 TreeBin 红黑树--双向链表 转换


### 5.8 CHM 基于相同key的oldValue 做一些操作
compute 
computeIfPresent
computeIfAbsent
...
### 6.0 CHM - merge(key, value, Function<oldValue, newValue>)
...
## 7 JUC - CopyOnWriteArrayList 并发集合

### 7.1 CopyOnWriteArrayList 介绍
- 线程安全的 集合ArrayList
...
## 10 异步编程- CompletableFuture 任务编排
...
### 11.1 **CPU 密集型** 和 **IO 密集型** 任务的线程数设置需遵循不同的规则

**关注点核心在 <font color=red>线程切换的消耗占比</font> 即 <font color=green>CPU处理任务的利用率</font>**

- **CPU 密集型任务**

  - 任务主要消耗 CPU 资源（如复杂计算、数据加密）。

  - **<font color=orange>线程数过多会导致频繁上下文切换，降低性能</font>**  因此 --- **<font color=red>线程数不需要太多</font>**

- **IO 密集型任务**

  - 任务频繁等待 IO（如数据库查询、网络请求、文件读写）。

  - **<font color=green>CPU 经常空闲，可增加线程数以提高吞吐量</font>**。因此 --- **<font color=red>线程数要多一些</font>**

**ps: 具体数量需要压测 + Hippo4j 测试所得。避免线程饥饿或过度竞争**





- **<font color=red>线程饥饿</font>**：**<font color=green>无法获取所需资源（如CPU时间片、锁、I/O等）而无法执行任务的现象</font>**

- **<font color=red>过度竞争</font>**：**<font color=blue>大量时间浪费在上下文切换和锁竞争上</font>**
